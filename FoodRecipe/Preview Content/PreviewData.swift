//
//  PreviewData.swift
//  FoodRecipe
//
//  Created by Raul Palade on 17/05/24.
//

import Foundation
import UIKit

var countryPreviewData = Country(id: 1, name: "Italy", isoAlpha2: "IT", flag: "")

var countriesPreviewData = [Country](repeating: countryPreviewData, count: 20)

let exampleRecipeImage = UIImage(named: "foodCardBg") ?? UIImage()
let exampleAuthorImage = UIImage(named: "profileAvatar") ?? UIImage()

let recipePreviewData = RecipeViewData(
    id: "1",
    name: "Pasta al pomodoro",
    description: "A simple and tasty dish, perfect for a quick dinner.",
    category: ["pranzo", "cena"],
    ingredients: [RecipeIngredient(name: "Pasta", quantity: "100g")],
    instructions: ["Cuocere la pasta"],
    nutrition: Nutrition(calories: 360, carbs: 60, proteins: 10, fats: 10),
    author: RecipeAuthorViewData(
        authorId: "",
        name: "Raul Palade",
        about: "Dedicated chef committed to using fresh, seasonal ingredients to create culinary masterpieces",
        image: exampleAuthorImage
    ),
    rating: 4.8,
    time: "20 min",
    createdAt: "",
    image: exampleRecipeImage
)

var recipesPreviewData = [RecipeViewData](repeating: recipePreviewData, count: 5)
